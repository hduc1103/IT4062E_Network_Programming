                   ...
                   else if (lower_choice1 == "search")
{
    ...
    send(client_socket, search_msg.c_str(), search_msg.length(), 0);
                    ...
                    {
                        size_t startPos = response1.find("Y_modified1");
                        size_t endPos = response1.find(';', startPos);
                        if (endPos != std::string::npos)
                        {
                            tmp_noti += response1.substr(startPos + 11, endPos - startPos) + "\n";
                        }
                    }

                    if (response1.find("Y_modified2") != std::string::npos)
                    {
                        size_t startPos = response1.find("Y_modified2");
                        size_t endPos = response1.find(';', startPos);
                        if (endPos != std::string::npos)
                        {
                            string tmp;
                            tmp += response1.substr(startPos + 11, endPos - startPos) + "\n";
                        }
                    }
                    if (response1.find("Y_modified3") != std::string::npos)
                    {
                        size_t startPos = response1.find("Y_modified3");
                        size_t endPos = response1.find(';', startPos);
                        if (endPos != std::string::npos)
                        {
                            tmp_noti += response1.substr(startPos + 11, endPos - startPos) + "\n";
